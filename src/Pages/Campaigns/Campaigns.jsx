...
const Campaigns = () => {
  ...
  const totalPages = Math.ceil(displayedCampaigns.length / cardsPerPage);

  const startIndex = currentPage * cardsPerPage;
  const endIndex = startIndex + cardsPerPage;

  const currentCampaigns = displayedCampaigns.slice(startIndex, endIndex);
        ...
        >
          <div className={`cards-wrapper ${showFilter ? 'with-filter' : ''}`}>
            {searchErr ? (
              ...
              <div className='filter-message'>جاري تطبيق الفلتر ...</div>
            ) : filterErr ? (
              <div className='filter-message error'>
          ...
          {showFilter && (
            <div className='filter-wrapper'>
              <CampaignFilter filters={filters} setFilters={setFilters} />
            </div>
          )}
        </section>
